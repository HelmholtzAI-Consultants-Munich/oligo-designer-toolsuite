...
sequence_file_ensemble = "data/annotations/custom_Homo_sapiens.GRCh38.dna_rm.primary_assembly_chr16.fa"
...
def test_region_generator_ncbi(tmpdir_factory):
    ...
    assert fasta_parser.check_fasta_format(ncbi_genes) == True, "error: wrong file format"

    ncbi_exons = region_generator_ncbi.get_sequence_exon()
    assert fasta_parser.check_fasta_format(ncbi_exons) == True, "error: wrong file format"
    ...
    assert fasta_parser.check_fasta_format(ncbi_junction) == True, "error: wrong file format"

    ncbi_intergenic = region_generator_ncbi.get_sequence_intergenic()
    assert fasta_parser.check_fasta_format(ncbi_intergenic) == True, "error: wrong file format"

    ncbi_introns = region_generator_ncbi.get_sequence_intron()
    assert fasta_parser.check_fasta_format(ncbi_introns) == True, "error: wrong file format"
...
def test_region_generator_ensemble(tmpdir_factory):
    ...
    assert fasta_parser.check_fasta_format(ensembl_gene) == True, "error: wrong file format"

    ensembl_exon = region_generator_ensembl.get_sequence_exon()
    assert fasta_parser.check_fasta_format(ensembl_exon) == True, "error: wrong file format"

    ensembl_CDS = region_generator_ensembl.get_sequence_CDS()
    assert fasta_parser.check_fasta_format(ensembl_CDS) == True, "error: wrong file format"

    ensembl_UTR = region_generator_ensembl.get_sequence_UTR(five_prime=True, three_prime=True)
    assert fasta_parser.check_fasta_format(ensembl_UTR) == True, "error: wrong file format"

    ensembl_junction = region_generator_ensembl.get_sequence_exon_exon_junction(block_size=50)
    assert fasta_parser.check_fasta_format(ensembl_junction) == True, "error: wrong file format"

    ensembl_intergenic = region_generator_ensembl.get_sequence_intergenic()
    assert fasta_parser.check_fasta_format(ensembl_intergenic) == True, "error: wrong file format"

    ensembl_intron = region_generator_ensembl.get_sequence_intron()
    assert fasta_parser.check_fasta_format(ensembl_intron) == True, "error: wrong file format"
