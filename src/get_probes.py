...
def get_mapping_gene_probes(dir_output, file_exon_sequence, mapping_exon_to_gene, probe_length, GC_content_min, GC_content_max, Tm_min, Tm_max, logging = None):
    ...
    if not os.path.isfile(file_mapping_gene_to_probes):
        ...
        if logging:
            logging.info('Total number of exons processed: {} with {} probes in total.'.format(count_exons, count_probes))
...
def get_unique_probes(dir_output, mapping_gene_to_probes, mapping_gene_to_probes_unique, logging=None):
