...
def args():
    ...
    args_parser.add_argument(
        "-o", "--output", help="path of output folder", type=str, required=True
    )
    args_parser.add_argument(
        "-d",
        "--download_only",
        help="dry run that only downloads the necessary files",
        action="store_true",
        required=False,
    )
...
def download_annotations(config, dir_output, download_only=False):
    ...
    t = (time.time() - t) / 60
    logging.info(
        "Time to load annotations, genes, transcriptome and probes: {} min".format(t)
    )
...
def filter_probes(config, datamodule, dir_output, dir_annotations=None):
    ...
    probefilter = ProbeFilter(
        config,
        dir_output,
        datamodule.file_transcriptome_fasta,
        datamodule.genes,
        dir_annotations,
    )
...
def design_padlock_probes(config, dir_output, dir_probes=None, dir_probesets=None):
    ...
    sequencedesigner = ProbeSequenceDesigner(
        config, dir_output, dir_probes, dir_probesets
    )
...
def main(config: DictConfig):
    ...
    if download_only:
        logging.info(
            "Download finished. Pipeline interrupted. Set download_only to False to run the full pipeline."
        )
        return
