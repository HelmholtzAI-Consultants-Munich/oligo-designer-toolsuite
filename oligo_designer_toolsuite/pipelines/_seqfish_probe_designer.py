...
class SeqFISHProbeDesigner:
    ...
    def create_probes(self):
        ...
        """
        logging.info(
            f"Create transcriptome "+str(self.config['source'])
        )
        ...
        self.oligo_database.remove_regions_with_insufficient_oligos(pipeline_step = "after applying property filters")
        if self.config["write_intermediate_steps"]:
            file_database = self.oligo_database.write_database(filename="oligo_database_property_filter.txt")
        
        logging.info(
            f"Property filters applied"
        )

        #SPECIFICITY FILTERS
        # Filters: ExactMatches and Blastn

        self.dir_specificity = os.path.join(self.dir_output, "specificity_temporary") # folder where the temporary files will be written
        ...
        self.oligo_database = specificity_filter.apply(oligo_database=self.oligo_database, reference_database=self.reference, n_jobs=self.config["n_jobs"])
        if self.config["write_intermediate_steps"]:
            file_database = self.oligo_database.write_database(filename="oligo_database_specificity_filter.txt")
        
        logging.info(
            f"Specificity filters applied"
        )
        ...
        barcodes_creator = BarcodingCreator(self.config["num_pseudocolors"], list(self.oligo_database.database.keys()))
        ...
        self.oligo_database.database = probes_creator.create_probes(self.oligo_database.database, self.readout_probes, barcodes_for_genes)
        logging.info(
            f"Total probes are built"
        )
        ...
        self.ref_db_cross_hybr = ReferenceDatabase(file_fasta = self.dir_output + '/oligo_database/fasta_from_our_db.fna')
        exact_mathces = ExactMatches(dir_specificity=self.dir_specificity)
        ...
        specificity_filter = SpecificityFilter(filters=filters, write_regions_with_insufficient_oligos=self.config["write_removed_genes"])
        self.oligo_database = specificity_filter.apply(oligo_database=self.oligo_database, reference_database=self.ref_db_cross_hybr, n_jobs=self.config["n_jobs"])
        
        logging.info(
            f"Cross-hybridization check is performed"
        )
        ...
        self.oligo_database = oligoset_generator.apply(oligo_database=self.oligo_database, n_sets=self.config["n_sets"], n_jobs=self.config["n_jobs"])
        if self.config["write_intermediate_steps"]:
            self.oligo_database.write_oligosets(dir_oligosets="oligosets")    
        logging.info(
            f"Oligoset generation is finished. Done!"
        )    
