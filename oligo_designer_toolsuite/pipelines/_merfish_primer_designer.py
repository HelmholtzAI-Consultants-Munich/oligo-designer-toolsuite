...
from oligo_designer_toolsuite.database import CustomReferenceDB, CustomOligoDB
...
class PrimerProbes:
    ...
    def __init__(
        ...
        #first blast against human genome
        ...
        self.reference_database1 = CustomReferenceDB(
        ...
        self.reference_database1.create_reference_DB(block_size=config["block_size"]) 
        ...
        self.reference_database3 = self.blast_filter2._create_fasta_file(T7_dict, self.dir_specificity3, 'T7')
        #ADD: create custom DB with fasta file
        '''
    ...
    def create_primer1(self):
        ...
        self.reference_database2 = self.blast_filter2._create_fasta_file(trimmed_primers, self.dir_specificity2, 'primers')
        #ADD: create custom reference DB with fasta file
        ...
        oligo_database = specificity_filter2.apply(oligo_database=oligo_database, reference_database=self.reference_database2, n_jobs=self.config["n_jobs"])
        if self.config["write_intermediate_steps"]:
            oligo_database.write_oligos_DB(format=self.config["file_format"], dir_oligos_DB="specificity_filter")
        ...
        # save fasta file with created primer indexes
        self.primer_database = self.blast_filter3._create_fasta_file(oligo_database, self.config["dir_output"], 'primerDB')
        '''

        return oligo_database[1:self.num_probes]
