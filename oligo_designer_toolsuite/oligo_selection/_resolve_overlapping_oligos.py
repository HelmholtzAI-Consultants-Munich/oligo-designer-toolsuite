...
class ProbesetGenerator:
    ...
    def _get_overlapping_matrix(self, probes):
        ...
        overlaps = [
            [
                _get_overlap(intervals[i], intervals[j])
                for i in range(len(intervals))
                if i > j
            ]
            for j in range(len(intervals))
        ]  # create half of the matrix (no diagonal) better put the contraint with j in the for loop directly?

        for i in range(len(overlaps)):
            overlaps[i].insert(0, True)
            overlaps[i] = [False] * (len(overlaps[0]) - len(overlaps[i])) + overlaps[
                i
            ]  # give the shape to the matrix

        overlaps = np.array([np.array(overlaps_i) for overlaps_i in overlaps]).astype(
            "int32"
        )
        overlaps = np.maximum(
            overlaps, overlaps.transpose()
        )  # complete the adjacency matrix with the correct values

        matrix = pd.DataFrame(
            overlaps, columns=probes.index, index=probes.index
    ...
    def _get_non_overlapping_sets(self, probes, overlapping_matrix, n_sets):
        ...
        G = nx.convert_matrix.from_numpy_matrix(overlapping_matrix.values)
        ...
        if (
            self.heurustic_selection is not None
        ):  # if not defined then it is not convenient to recomputhe the cliques
            # apply the heuristic
        ...
        #    columns=[f"probe_{i}" for i in range(n)] + ["set_score"]
        # )
        probesets = []
        ...
        for count, clique in enumerate(cliques):
            ...
            if count > 10000:  # set a meaningful value
                break
            if len(clique) >= n:
        ...
        if heuristic_probeset:
            best_score = probesets["set_score"].min()
            print(best_score)
            if heuristic_probeset[-1] < best_score:
...
def padlock_heuristic_selection(probes, overlapping_matrix, n_probe, n_trials=10):
    ...
    for first_idx in range(
        ...
        if len(set_idxs) == n_probe:
            score = probes_sorted.iloc[set_idxs[-1]][
                "probe_score"
            ]  # the df is sorted, no necessary to compute the max again
            if score < max_score:
                max_score = score
